...
class ModuleFields extends Eloquent
{
    ...
    public static function getModuleFields($moduleName)
    {
        ...
        $fields = DB::collection('module_fields')->where('module', $module->id)->get();
        $ftypes = ModuleFieldTypes::getFTypes();
        ...
        if(Schema::hasTable($external_table_name)) {
            if(substr($value_id, 0, 1) == '[' && substr($value_id, -1) == ']'){
                ...
                for ($i = 0; $i < $tot; $i++){
                    ...
                    $id = substr($id, 0, -1);
                    $external_value = DB::collection($external_table_name)->where('id', '9f3acd77-3ab5-492a-ae9d-c2a3be0a0945')->first();
                    $external_module = DB::collection('modules')->where('name_db', $external_table_name)->first();
                ...
                $external_value = DB::collection($external_table_name)->where('id', $value_id)->get();
